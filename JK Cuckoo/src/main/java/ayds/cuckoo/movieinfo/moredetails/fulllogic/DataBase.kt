        ...
        try {
            DriverManager.getConnection(url).use { connection ->
                if (connection != null) {
                    ...
                    statement.executeUpdate("create table info (id INTEGER PRIMARY KEY AUTOINCREMENT, title string, summary string, image_url string, source integer)")
                }
            }
...
// it probably means no database file is found
    ...
    @JvmStatic
    fun saveMovieInfo(title: String, summary: String, imageUrl: String) {
        var connection: Connection? = null
        try { // create a database connection
            connection = DriverManager.getConnection("jdbc:sqlite:./extra_info.db")
            val statement = connection.createStatement()
            statement.queryTimeout = 30 // set timeout to 30 sec.
            println("INSERT  $title', '$summary', '$summary")
            statement.executeUpdate("insert into info values(null, '$title', '$summary', '$imageUrl', 1)")
        } catch (e: SQLException) {
    ...
    @JvmStatic
    fun getSummary(title: String): String? {
        var connection: Connection? = null
        try { // create a database connection
            connection = DriverManager.getConnection("jdbc:sqlite:./extra_info.db")
            val statement = connection.createStatement()
